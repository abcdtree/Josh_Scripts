...
def st_id_tab_prepare(st, logger):
	if os.path.exists("./input.tab"):
		os.remove("./input.tab")
	for my_st in st:
		cmd = f"mdu-query --st {my_st} --species Salmonella | cut -f1 | tail -n +2 | sort > temp.list"
		bash_command(cmd)
		#print(cmd)
		mdu_cmd = f"mdu-reads --idfile temp.list --skip >> input.tab"
		#print(mdu_cmd)
...
def check_st(df_window, date_str, logger):
	qc_database = ""
	if os.path.exists("/home/seq/MDU/QC/QC.tab"):
		qc_database = "/home/seq/MDU/QC/QC.tab"
	else:
		qc_database = "/home/seq/MDU/QC/QC.tab.busy"
