...
def run_bohra(reference, mask, cpu, input_tab, job_id, b_type):
	...
	cmd = ""
	if mask == None:
		cmd = f"bohra run --cpus {cpu} -i {input_tab} -j {job_id} -r {reference}\
		 -p {b_type} --proceed"
	else:
		cmd = f"bohra run --cpus {cpu} -i {input_tab} -j {job_id} -r {reference}\
		 -m {mask} -p {b_type} --proceed"
	bash_command(cmd)
	return
...
def main():
	...
	parser.add_argument("--bohra", "-b", help="Model of Bohra Pipeline: preview, pluspan",\
						default="pluspan")
